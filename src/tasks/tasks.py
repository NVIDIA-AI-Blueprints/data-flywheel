...
def create_datasets(
    ...
    Args:
        ...
        data_split_config: Configuration for dataset splitting, including:
            - eval_size: Number of records for evaluation (default: 500)
    ...
    eval_records, train_records, val_records = split_records(records, settings.data_split_config)
    logger.info(
...
def spin_up_llm_judge(previous_result: TaskResult) -> TaskResult:
    ...
    def progress_callback(status: str):
        db.llm_judge_runs.update_one(
            {"_id": llm_judge_run.id}, {"$set": {"deployment_status": DeploymentStatus(status)}}
        )
...
def spin_up_nim(previous_result: TaskResult, nim_config: dict) -> TaskResult:
    ...
    if not dms_client.is_deployed():
        ...
        try:
            ...
            db.nims.update_one({"_id": nim_run.id}, {"$set": {"status": NIMRunStatus.ERROR}})
            raise e
    else:
    ...
    def progress_callback(status: str):
        db.nims.update_one(
            {"_id": nim_run.id}, {"$set": {"deployment_status": DeploymentStatus(status)}}
        )
...
def run_generic_eval(
    ...
    """
    logger.info("Running ICL evaluation")
    ...
    for job in jobs:
        ...
        if previous_result.workload_type == WorkloadClassification.TOOL_CALLING:
            if results["tasks"]["custom-tool-calling"]:
                ...
                scores["function_name_and_args_accuracy"] = results["tasks"]["custom-tool-calling"][
                    "metrics"
                ]["tool-calling-accuracy"]["scores"]["function_name_and_args_accuracy"]["value"]
