...
def load_data_to_elasticsearch(
    workload_id: str = "", client_id: str = "", file_path: str = "bug_triage_dataset.jsonl"
):
    ...
    if test_data[0].get("workload_id"):
        # Document is already in the correct format
        print("Document is already in the log format. Loading as-is.")
        for doc in test_data:
            es.index(index=ES_COLLECTION_NAME, document=doc)
    else:
    ...
    es.indices.flush(index=ES_COLLECTION_NAME)
