...
from notebooks.utils.mlflow_helper import (
    download_and_process_eval,
    ...
    for nim in job_data["nims"]:
        ...
        for eval in nim["evaluations"]:
            ...
            row = {
                ...
                "Started": datetime.fromisoformat(eval["started_at"]).strftime(
                    "%H:%M:%S"
                ),
                "Finished": (
                    datetime.fromisoformat(eval["finished_at"]).strftime("%H:%M:%S")
                    if eval["finished_at"]
                    else "-"
                ),
            }
            ...
            if "function_name_and_args_accuracy" in all_scores:
                row["Function name + args accuracy (exact-match)"] = all_scores[
                    "function_name_and_args_accuracy"
                ]

            if "tool_calling_correctness" in all_scores:
                row["Function name + args accuracy (LLM-judge)"] = all_scores[
    ...
    if not rows:
        return pd.DataFrame(
            columns=[
                ...
                "Tool Calling Correctness (LLM-Judge)",
    ...
    for nim in job_data["nims"]:
        ...
        for custom in nim["customizations"]:
            customizations.append(
                {
                    "Model": model_name,
                    "Started": datetime.fromisoformat(custom["started_at"]).strftime(
                        "%H:%M:%S"
                    ),
                    "Epochs Completed": custom["epochs_completed"],
                    "Steps Completed": custom["steps_completed"],
                    "Finished": (
                        datetime.fromisoformat(custom["finished_at"]).strftime(
                            "%H:%M:%S"
                        )
                        if custom["finished_at"]
                        else "-"
                    ),
...
def monitor_job(
    api_base_url, job_id, poll_interval, mlflow_uri=None, enable_mlflow_upload=False
):
    ...
    if enable_mlflow_upload:
        ...
        print(
            f"MLflow integration enabled - will upload completed evaluations to {mlflow_uri}"
        )
            ...
            if enable_mlflow_upload:
                ...
                for nim in job_data["nims"]:
                    ...
                    for eval_data in nim["evaluations"]:
                        ...
                        if (
                            eval_data.get("finished_at")
                            and eval_key not in completed_evaluations
                        ):
                            ...
                            if nmp_eval_uri:
                                ...
                                try:
                                    ...
                                    if results_json:
                                        upload_result_to_mlflow(
                                            results_json, tracking_uri=mlflow_uri
                                        )
                                        print(
                                            f"✅ Successfully uploaded {eval_key} to MLflow"
                                        )
                                    else:
                                        print(
                                            f"❌ Failed to download results for {eval_key}"
                                        )
                                ...
                                print(
                                    f"⚠️ No nmp_uri or eval_id found for completed evaluation: {eval_key}"
                                )
